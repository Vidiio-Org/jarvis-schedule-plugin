...
export class AttachmentStore {
    ...
    save(scheduleId, input, existing) {
        if (typeof input.name !== 'string' || input.name.trim() === '')
            throw new ValidationError('name: is required');
        if (typeof input.data !== 'string' || input.data === '')
            throw new ValidationError('data: is required (base64, no "data:" prefix)');
        let mime = DEFAULT_MIME;
        if (input.mime !== undefined && input.mime !== null && input.mime !== '') {
            if (typeof input.mime !== 'string' || !MIME_PATTERN.test(input.mime.trim()))
                throw new ValidationError('mime: must look like "image/png"');
        ...
        if (bytes.length === 0)
            throw new ValidationError('data: the file is empty');
