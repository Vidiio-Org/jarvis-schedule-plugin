...
import { NotFoundError, type ScheduleService } from './service.js';
import type { LogFn } from './store.js';
...
export class ApiServer {
    ...
    return new Promise((resolve, reject) => {
      const server = createServer((req, res) => {
        void this.handle(req, res).catch((err) => {
          ...
          if (!res.headersSent) this.sendJson(res, 500, { ok: false, code: 'INTERNAL', message: 'Internal error' });
          else res.end();
        });
      });
  ...
  private hostAllowed(host: string | undefined): boolean {
    ...
    if (!m) return false;
    if (!ALLOWED_HOSTNAMES.has(m[1] ?? '')) return false;
  ...
  private sendError(res: ServerResponse, err: unknown): void {
    if (err instanceof HttpError) return this.sendJson(res, err.status, { ok: false, code: err.code, message: err.message });
    if (err instanceof ValidationError) return this.sendJson(res, 400, { ok: false, code: 'VALIDATION_ERROR', message: err.message });
    if (err instanceof PayloadTooLargeError) return this.sendJson(res, 413, { ok: false, code: 'PAYLOAD_TOO_LARGE', message: err.message });
    if (err instanceof NotFoundError) return this.sendJson(res, 404, { ok: false, code: 'NOT_FOUND', message: err.message });
  ...
  private async route(method: string, url: URL, req: IncomingMessage): Promise<[number, unknown]> {
    const svc = this.opts.service;
    const path = url.pathname.replace(/\/+$/, '');
    const only = (allowed: string): void => {
      if (method !== allowed) throw new HttpError(405, 'METHOD_NOT_ALLOWED', `Use ${allowed} for ${path}.`);
    };
    ...
    if (path === '/api/schedules') {
      if (method === 'GET') return [200, await svc.listSchedules()];
      if (method === 'POST') return [201, await svc.createSchedule(await this.readBody(req))];
    ...
    if (attachments) {
      ...
      return [201, await svc.addAttachment(decodeURIComponent(attachments[1]!), body as { name: unknown; mime: unknown; data: unknown })];
    }
    ...
    if (one) {
      ...
      if (method === 'PUT') return [200, await svc.updateSchedule(id, await this.readBody(req))];
      if (method === 'DELETE') {
  ...
  private async serveStatic(method: string, rawPath: string, res: ServerResponse): Promise<void> {
    ...
    if (bad) return this.notFound(res, decoded === '/');

    try {
      const root = await realpath(this.opts.uiDir);
      const file = await realpath(join(root, ...segments));
      if (!file.startsWith(root + sep)) return this.notFound(res, false);
      const info = await stat(file);
      if (!info.isFile()) return this.notFound(res, false);
  ...
  private notFound(res: ServerResponse, isRoot: boolean): void {
    if (isRoot) {
      const html =
        '<!doctype html><meta charset="utf-8"><title>Jarvis Schedule</title><p>The dashboard UI is not installed (ui/index.html is missing). The API is available under /api.</p>';
