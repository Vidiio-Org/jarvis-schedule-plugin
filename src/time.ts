...
export function offsetMs(ms: number, timeZone: string): number {
...
export function zonedWallToUtc(
  ...
  const matches = (utc: number): boolean => {
...
export function dateString(p: { year: number; month: number; day: number }): string {
...
export interface Slot {
  /** Stable dedupe key: `<scheduleId>|<local YYYY-MM-DD>|<HH:MM>`. */
  key: string;
  /** Local date (in the schedule's timezone) the slot belongs to. */
  date: string;
  /** The instant the slot is due, epoch ms. */
  at: number;
}

/** Every slot of `spec` with `fromExclusive < at <= toInclusive`, ascending. */
export function slotsBetween(spec: SlotSpec, fromExclusive: number, toInclusive: number): Slot[] {
  const match = TIME_PATTERN.exec(spec.time);
  if (!match || toInclusive <= fromExclusive) return [];
  ...
  for (let t = firstDay; t <= lastDay; t += DAY_MS) {
    const d = new Date(t);
    if (!spec.days.includes(d.getUTCDay())) continue;
    const y = d.getUTCFullYear();
    const m = d.getUTCMonth() + 1;
    const day = d.getUTCDate();
    const at = zonedWallToUtc(y, m, day, hour, minute, spec.timezone);
    if (at <= fromExclusive || at > toInclusive) continue;
...
export function nextSlotAfter(spec: SlotSpec, now: number): Slot | null {
